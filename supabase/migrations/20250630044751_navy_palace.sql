/*
  # Add departments and notification preferences
  ...
  2. New Functions
    ...
    - `auto_assign_complaint_department` - Automatically assign complaints to departments
    - `assign_user_to_department` - Assign a user to a department

  3. New View
    - `department_stats` - View for department statistics
...
BEGIN
  ...
  IF NEW.category = 'sanitation' THEN 
    dept_name := 'Health';
  ELSIF NEW.category = 'infrastructure' THEN 
    dept_name := 'Public Works';
  ELSIF NEW.category = 'publicServices' THEN 
    IF NEW.description ILIKE '%school%' OR NEW.description ILIKE '%education%' THEN 
      dept_name := 'Education';
    ELSE 
      dept_name := 'Public Works';
    END IF;
  ELSIF NEW.category = 'utilities' THEN 
    dept_name := 'Utilities';
  ELSIF NEW.category = 'transportation' THEN 
    dept_name := 'Transportation';
  ELSIF NEW.category = 'other' THEN 
    -- For 'other', check description for keywords
    IF NEW.description ILIKE '%environment%' OR NEW.description ILIKE '%pollution%' THEN 
      dept_name := 'Environment';
    ELSIF NEW.description ILIKE '%water%' OR NEW.description ILIKE '%electricity%' THEN 
      dept_name := 'Utilities';
    ELSIF NEW.description ILIKE '%road%' OR NEW.description ILIKE '%transport%' THEN 
      dept_name := 'Transportation';
    ELSE 
      dept_name := 'Public Works'; -- Default department
    END IF;
  ELSE 
    dept_name := 'Public Works'; -- Default department
  END IF;
...
-- Create a function to assign a department to a user (with a different name to avoid conflict)
CREATE OR REPLACE FUNCTION assign_user_to_department(user_id uuid, dept_name text)
RETURNS void AS $$
BEGIN
